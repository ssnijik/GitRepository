...
public class CourseService extends BaseService<Course, Integer> implements
	...
	public boolean addCourse(Date startTime, Date endTime, int duration,
		...
		Course course = new Course(startTime, endTime, duration,
				user.getTutor(), price, name, description, subject);
	...
	@Override
	public void updateRemark(int courseid, int ebta, int ebtb, int ebtc,
			int ebsa, int ebsb, int ebsc, String evaluation) {
	...
	@Override
	public List<Course> getCoursesByTutor(String email, int pageNumber,
			int pageSize) {
		User queryUser = new User();
		queryUser.setEmail(email);
		User user = userDao.queryByCriteriaUnique(queryUser);
		...
		List<Course> courseList = courseDao.listAll(pageNumber, pageSize,
				"model.tutor.id='" + tutor.getId() + "'");
	...
	public List<Course> getCoursesBySubject(String subjectName, int pageNumber,
		...
		List<Course> courseList = courseDao.listAll(pageNumber, pageSize,
				"model.subject.id='" + subject.getId() + "'");
	...
	public void addStudent(int courseid, int studentid) {
		...
		for (Student s : course.getApplications()) {
			s.getApplications().remove(course);
			notificationDao.save(new Notification(courseid, 'a' + 'd', student
					.getUser()));
		}
		course.getApplications().clear();
		// courseDao.update(course);
		notificationDao.save(new Notification(courseid, 'a' + 'a', student
				.getUser()));
		notificationDao
				.save(new Notification(courseid, 'c', student.getUser()));
	}
